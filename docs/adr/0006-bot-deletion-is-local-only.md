...
Permanent Bot deletion owns only data managed by Omarchy Bot: the Bot, Threads, messages, Turns, attachments, avatar files, and local Agent-session mappings. Agent-owned Native Sessions and Shared Workspace files are outside this boundary and survive deletion. The confirmation must say that deleting a Bot is not an Agent-data privacy erase.
