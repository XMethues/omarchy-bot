...
Each Bot retains one Bot Screen identity. Its first graphical action or requested desktop view starts a lightweight, pure-headless Cage Bot Desktop Session on demand, with its own display endpoint and independent windows, focus, pointer, and keyboard state. The Host Session remains the user's original Omarchy environment. Bot A can keep operating its own graphical applications while the client selects Bot B; the client replaces its Screen Projection and input connection, not the underlying Bot's work.
...
- Retain Cage as the existing sole compositor, with one pure-headless instance per running Bot Desktop Session. Do not add a compositor selector, full Omarchy/UWSM session, host-workspace projection, VNC replacement, or SSH-based pseudo-isolation.
- Reuse the Bot Screen management module, Cage runtime adapter, computer worker, application-unit supervision, Screen Projection module, and Computer Broker. Keep private runtime/display endpoints and explicit per-Screen input routing; evolve these existing interfaces rather than creating a parallel desktop subsystem.
...
- Each client displays only its selected Screen. Switching or closing releases that client's old projection, stale content, and input authority without cancelling background work, destroying the old desktop, or completing an outstanding Takeover. Cleanup considers remaining viewers before stopping resources still in use.
- Preserve low-frequency lossless Computer Preview, H.264 expanded Web Control, existing signaling/input authority semantics, and explicit HTTP snapshot fallback. Stop continuous viewer-driven capture/encoding when there are no viewers and release expanded-only encoding on mode exit. Agent-requested screenshots remain valid without a viewer.
...
- Preserve generation, geometry, held-input release, stale-message rejection, per-Screen Takeover, and capacity-admission protections. Do not silently raise capacity limits or introduce idle eviction that discards applications or unsaved work.
...
- Test prior art: [daemon integration harness](../../tests/integration/helpers/harness.ts), [Computer browser flows](../../tests/e2e/specs/10-computer-sheet.spec.ts), [deletion browser flows](../../tests/e2e/specs/11-permanent-deletion.spec.ts), [responsive/accessibility coverage](../../tests/e2e/specs/13-responsive-accessible-visual-qa.spec.ts), [mailbox worker observations](../../tests/integration/bot-mailbox.test.ts), [Screen lifecycle](../../tests/integration/bot-screen-lifecycle.test.ts), [real Cage smoke](../../tests/integration/bot-screen-cage.smoke.test.ts), and [capacity/load harness](../../tests/integration/bot-screen-capacity.load.test.ts). These are prior art, not claims that this specification's new acceptance already passes.
