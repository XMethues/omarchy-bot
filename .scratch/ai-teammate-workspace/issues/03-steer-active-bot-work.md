...
The Pi adapter accepts steering only for its currently running streaming turn and delegates exclusively to native `session.steer`. The fake Pi worker models a deterministic atomic-action boundary: it acknowledges the steer, completes the active tool action, then applies the queued redirect. Explicit abort remains a separate cancellation path and retains its reason. The real-Pi conformance scenario is defined separately from the feature's completion status.
